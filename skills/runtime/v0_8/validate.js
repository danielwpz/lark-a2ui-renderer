...
import { DYNAMIC_DATA_EXTENSION_ID, LARK_CARD_CATALOG_ID, LARK_CARD_LIVE_CATALOG_ID, } from "./types.js";
...
export function validateA2uiMessages(input, options = {}) {
    ...
    if (!hasErrors(issues)) {
        try {
            ...
            store.applyMessages(input.filter(isCoreServerMessage));
...
function validateMessage(message, index, issues, beginSurfaceIds, componentIdsBySurface, options) {
    ...
    if (key === "surfaceUpdate") {
        validateSurfaceUpdate(message.surfaceUpdate, `${path}.surfaceUpdate`, issues, componentIdsBySurface);
        return;
    }
    ...
    if (key === "beginRendering") {
        validateBeginRendering(message.beginRendering, `${path}.beginRendering`, issues, beginSurfaceIds, options);
        return;
    }
...
function validateSurfaceUpdate(value, path, issues, componentIdsBySurface) {
    ...
    const seenIds = surfaceId == null ? new Set() : ensureComponentSet(componentIdsBySurface, surfaceId);
...
function validateBeginRendering(value, path, issues, beginSurfaceIds, options) {
    ...
    if (value.catalogId !== undefined &&
        ...
        addError(issues, `${path}.catalogId`, `catalogId must be '${LARK_CARD_CATALOG_ID}' or '${LARK_CARD_LIVE_CATALOG_ID}' for this renderer`);
    }
}
function validateDataSourceUpdate(value, path, issues) {
    if (!isRecord(value)) {
    ...
    if (value.extensionId !== undefined && value.extensionId !== DYNAMIC_DATA_EXTENSION_ID) {
        addError(issues, `${path}.extensionId`, `extensionId must be '${DYNAMIC_DATA_EXTENSION_ID}' when provided`);
    }
...
function validateRenderedSurface(surface, issues) {
    ...
    for (const node of surface.components.values()) {
        ...
        if (component.type === "Grid" && surface.catalogId !== LARK_CARD_LIVE_CATALOG_ID) {
            addError(issues, `$.surface(${surface.surfaceId}).component(${node.id})`, `Grid requires catalogId '${LARK_CARD_LIVE_CATALOG_ID}'`);
        }
        ...
        if (component.type === "TextField" ||
            ...
            if (inputNames.has(name)) {
                addError(issues, `$.surface(${surface.surfaceId}).component(${node.id}).name`, `Duplicate form field name '${name}'`);
            }
            inputNames.add(name);
        }
    }
...
function validateChildReferences(surface, value, ownerId, issues) {
    ...
    for (const childId of childIds) {
        if (!surface.components.has(childId)) {
            addError(issues, `$.surface(${surface.surfaceId}).component(${ownerId}).children`, `Unknown child component '${childId}'`);
        }
    }
}
function assertReferencedType(surface, componentId, expectedType, ownerId, field, issues) {
    if (!surface.components.has(componentId)) {
        addError(issues, `$.surface(${surface.surfaceId}).component(${ownerId}).${field}`, `Unknown referenced component '${componentId}'`);
        return;
    }
    const ref = readComponentRef(surface, componentId);
    if (ref.type !== expectedType) {
        addError(issues, `$.surface(${surface.surfaceId}).component(${ownerId}).${field}`, `${field} must reference a ${expectedType} component, got ${ref.type}`);
    }
}
...
function validateDataSourceDeclaration(value, path, issues) {
    if (!isRecord(value)) {
...
function isCoreServerMessage(message) {
...
function validateDataEntry(value, path, issues) {
    ...
    const valueFields = ["valueString", "valueNumber", "valueBoolean", "valueMap"].filter((field) => field in value);
