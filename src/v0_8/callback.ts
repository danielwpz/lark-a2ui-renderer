...
export function extractLarkCallback(rawPayload: unknown): NormalizedCallbackInput {
  ...
  const input: NormalizedCallbackInput = {
    envelope,
  };
...
function readLarkCallbackEvent(root: Record<string, unknown>): Record<string, unknown> {
...
function readCallbackEnvelope(value: unknown): CallbackEnvelope {
...
function parseCallbackValue(value: unknown): unknown {
...
function readSubmittedValues(action: Record<string, unknown>): Record<string, unknown> | undefined {
...
function readCallbackTimestamp(root: Record<string, unknown>): string | undefined {
...
function validateEnvelope(surface: SurfaceState, envelope: CallbackEnvelope): void {
...
function readBoundPath(ref: { type: string; props: Record<string, unknown> }): string | null {
  const binding =
    ref.type === "TextField"
      ? ref.props.text
...
function readButtonAction(value: unknown): ButtonAction {
  ...
  if (Array.isArray(value.context)) {
    action.context = value.context as ActionContextEntry[];
  }
  return action;
}
